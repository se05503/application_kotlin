...
class QuestionFragment : Fragment() {
    ...
    private var questionType: Int = 0 // questionType : 몇번째 페이지인가? 첫번째 페이지인 경우 0 이다.
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val btn_next : Button = view.findViewById(R.id.btnNext)
        btn_next.setOnClickListener {
            // 라디오 버튼이 모두 체크가 된 경우 -> 변수 값은 True / 안된 경우 -> 변수 값은 False
            val isAllAnswered = answerRadioGroup.all { it.checkedRadioButtonId != -1 }

            if(isAllAnswered) {
                // 모든 질문에 답한 경우 → 다음 화면으로 넘어가야함
                val responses = answerRadioGroup.map { radioGroup ->
                    val firstRadioButton = radioGroup.getChildAt(0) as RadioButton
                    if(firstRadioButton.isChecked) 1 else 2 //체크가 되어있으면 1, 안되어있으면 2를 대입한다.
                } // response 의 값은 <1,1,2> 이런 식으로 들어간다.

                // TestActivity에 있는 fun addResponses(response:List<Int>) 를 불러야 한다.
                (activity as? TestActivity)?.questionaireResults?.addResponses(responses)
                (activity as? TestActivity)?.moveToNextQuestion() // 다음 화면으로 넘어가는 함수를 다른 액티비티에서 불러옴

            } else {
                // 모든 질문에 답하지 않은 경우 → 에러 메세지 띄우기
                // 여기서는 this 가 아니라 context를 해야한다 (이유?)
